...
/* DPPI instance for nRF5340 Application Core */
static nrfx_dppi_t dppi_instance = NRFX_DPPI_INSTANCE(0);
...
static int audio_sync_timer_init(void)
{
	...
	ret = nrfx_dppi_channel_alloc(&dppi_instance, &dppi_channel_i2s_frame_start);
	if (ret - NRFX_ERROR_BASE_NUM) {
	...
	ret = nrfx_dppi_channel_enable(&dppi_instance, dppi_channel_i2s_frame_start);
	if (ret - NRFX_ERROR_BASE_NUM) {
	...
	ret = nrfx_dppi_channel_alloc(&dppi_instance, &dppi_channel_curr_time_capture);
	if (ret - NRFX_ERROR_BASE_NUM) {
	...
	ret = nrfx_dppi_channel_enable(&dppi_instance, dppi_channel_curr_time_capture);
	if (ret - NRFX_ERROR_BASE_NUM) {
	...
	ret = nrfx_dppi_channel_alloc(&dppi_instance, &dppi_channel_rtc_start);
	if (ret - NRFX_ERROR_BASE_NUM) {
	...
	ret = nrfx_dppi_channel_enable(&dppi_instance, dppi_channel_rtc_start);
	if (ret - NRFX_ERROR_BASE_NUM) {
	...
	ret = nrfx_dppi_channel_alloc(&dppi_instance, &dppi_channel_timer_sync_with_rtc);
	if (ret - NRFX_ERROR_BASE_NUM) {
		LOG_ERR("nrfx DPPI channel alloc error (timer clear): %d", ret);
		return -ENOMEM;
	}
	...
	ret = nrfx_dppi_channel_enable(&dppi_instance, dppi_channel_timer_sync_with_rtc);
	if (ret - NRFX_ERROR_BASE_NUM) {
		LOG_ERR("nrfx DPPI channel enable error (timer clear): %d", ret);
		return -EIO;
	}
